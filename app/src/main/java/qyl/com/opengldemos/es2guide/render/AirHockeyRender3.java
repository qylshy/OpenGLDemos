...
import qyl.com.opengldemos.es2guide.objects.Puck;
...
public class AirHockeyRender3 implements GLSurfaceView.Renderer {
    ...
    private void divideByW(float[] vertor) {
        vertor[0] /= vertor[3];
        vertor[1] /= vertor[3];
        vertor[2] /= vertor[3];
    ...
    private Ray convertNormalized2DPointToRay(float normalizedX, float normalizeY) {
        final float[] nearPointNdc = {normalizedX, normalizeY, -1, 1};
        final float[] farPointNdc = {normalizedX, normalizeY, 1, 1};
